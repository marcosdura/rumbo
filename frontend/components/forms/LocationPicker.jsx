...
export default function LocationPicker({ lat, lng, onLocationSelect }) {
  ...
  const inputBorder = linkError ? "#e53e3e" : "var(--border)"
  ...
  return (
    <div style={{
      ...
      <p style={{
        ...
        color: lat != null ? "var(--primary)" : "var(--muted)",
      }}>
        {lat != null && lng != null
      ...
      />
      {resolving  && <p style={{ fontSize: 12, color: "var(--muted)", marginTop: 6 }}>Resolviendo link...</p>}
      {linkError  && <p style={{ fontSize: 12, color: "#e53e3e", marginTop: 6 }}>{linkError}</p>}
      {linkSuccess && <p style={{ fontSize: 12, color: "var(--primary)", marginTop: 6 }}>✓ Ubicación encontrada</p>}
    </div>
  )
}
